...
import CartAndWishlistItemsRow from "../Cart/CartAndWishlistItemsRow";
...
const ModalOverlay = (props) => {
  const items = props.items.map((item) => {
    return <CartAndWishlistItemsRow item={item} name={props.name} />;
  });
  ...
  return (
    <nav className={Classes.modal}>
      ...
      {props.name === "Cart" &&<div>
        ...
        <p className="text-slate-500 text-sm mb-12">Free Shipping on All Orders Over $100!</p>
      </div>}
    </nav>
  );
};
